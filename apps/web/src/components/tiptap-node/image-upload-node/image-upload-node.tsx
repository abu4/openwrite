...
// Regex constant for extracting file extensions
...
function useFileUpload(options: UploadOptions) {
  ...
  const createFileItem = (file: File): { fileItem: FileItem; abortController: AbortController } => {
    ...
    return { fileItem, abortController }
  }

  const updateFileProgress = (fileId: string, progress: number) => {
    setFileItems((prev) => prev.map((item) => (item.id === fileId ? { ...item, progress } : item)))
  }

  const markFileAsSuccess = (fileId: string, url: string) => {
    setFileItems((prev) =>
      prev.map((item) =>
  ...
  const markFileAsError = (fileId: string) => {
    setFileItems((prev) =>
  ...
  const performUpload = async (
    ...
    const url = await options.upload(
      file,
      (event: { progress: number }) => updateFileProgress(fileId, event.progress),
  ...
  const handleUploadSuccess = (fileId: string, url: string): string => {
    markFileAsSuccess(fileId, url)
    options.onSuccess?.(url)
    return url
  }

  const handleUploadError = (fileId: string, error: unknown): void => {
    markFileAsError(fileId)
  ...
  const uploadFile = async (file: File): Promise<string | null> => {
    ...
    const { fileItem, abortController } = createFileItem(file)
    setFileItems((prev) => [...prev, fileItem])

    try {
      const url = await performUpload(file, fileItem.id, abortController)
      ...
      if (url) {
        return handleUploadSuccess(fileItem.id, url)
      }

      return null
    } catch (error) {
      if (!abortController.signal.aborted) {
        handleUploadError(fileItem.id, error)
      }
      return null
    }
  }
...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
    ...
    if (urls.length > 0) {
      ...
      if (isValidPosition(pos)) {
        const imageNodes = urls.map((url, index) => {
          const filename = files[index]?.name.replace(FILE_EXTENSION_REGEX, "") || "unknown"
          return {
            type: "image",
  ...
  return (
    <NodeViewWrapper className="tiptap-image-upload">
      <button
        aria-label="Upload image files"
        className="tiptap-image-upload-button"
        onClick={handleClick}
